...
def get_property():
    ...
    try:
        import streamlit as st  # only needed here

        db_path = st.secrets.get("db_path", None)
        gemini_api_key = st.secrets.get("gemini_api_key", None)

        if db_path and gemini_api_key:
            print("Database path:", db_path)  # OK to log
            return db_path, gemini_api_key
    except Exception:
        # If Streamlit isn't available (or secrets not set), fallback to file
        pass
    ...
    # IMPORTANT: do NOT print the API key
    return db_path, gemini_api_key
